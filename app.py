...
def verify_carrier():
    ...
    try:
        ...
        fmcsa_only = data.get('fmcsa_only', False)  # Force FMCSA only, no fallback
        ...
        if use_fmcsa and FMCSA_API_KEY != 'your_fmcsa_api_key_here':
            ...
            if fmcsa_result['success']:
                ...
                # FMCSA failed, return the error instead of falling back to mock data
                return jsonify({
                    'success': False,
                    'verified': False,
                    'error': fmcsa_result.get('error', 'FMCSA API request failed'),
                    'message': 'Failed to retrieve data from FMCSA database',
                    'data_source': 'FMCSA',
                    'mc_number': mc_number
                }), 404
